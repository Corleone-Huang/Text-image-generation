...
def drawCaption(convas, captions, ixtoword, vis_size, off1=2, off2=2):
    num = captions.size(0)
    img_txt = Image.fromarray(convas)  # 实现array到image的转换
    # get a font 得到一个字体
    # fnt = None  # ImageFont.truetype('Pillow/Tests/fonts/FreeMono.ttf', 50)
    fnt = ImageFont.truetype(fm.findfont(fm.FontProperties(family='DejaVu Sans')),50)
    # fnt = ImageFont.truetype('Pillow/Tests/fonts/freemono.ttf', 50)
    # fnt = ImageFont.truetype('Pillow/Tests/fonts/FreeMono.ttf', 50)
    # get a drawing context 获取绘图上下文
    d = ImageDraw.Draw(img_txt)
    sentence_list = []
    for i in range(num): # in captions.szie(0)
